...
type LayerProps = { canvas: CanvasType, dimension: Dimension, cursor: string, canvasScale: number, isGridVisible: boolean }

function Layer({ canvas, dimension, cursor, canvasScale, isGridVisible }: LayerProps) {
    return (
        <div
            data-testid='canvas'
            className={`grid justify-center gap-0 select-none ${cursor}} absolute top-0 bottom-0 left-0 right-0 pointer-events-none`}
            style={{
...
function ActiveLayer({ canvas, dimension, updateCanvas, cursor, canvasScale, isGridVisible }: ActiveLayerProps) {
