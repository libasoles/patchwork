    ...
    return (
        <div className="flex justify-center items-center fixed  top-3 z-10">
            <div className="flex items-center space-x-4 bg-gray-800 rounded-full p-1">
                <Action name="draw" selected={selected} setSelected={setSelected}>
                ...
                <Action name="paint" selected={selected} setSelected={setSelected}>
...
function Action({ name, selected, setSelected, children }: Props) {
    const highlight = useMemo(() => {
        return (expected: string) => selected === expected ? 'bg-blue-500 text-white' : 'bg-white text-gray-800'
    }, [selected]);
